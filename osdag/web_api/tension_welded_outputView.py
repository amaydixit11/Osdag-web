...
class TensionWeldedOutputData(APIView):

    def post(self, request):
        print("Inside post method of TensionWeldedOutputData")

        # obtaining the session, module_id, input_values
        cookie_id = request.COOKIES.get('tension_welded_connection_session')
        module_api = get_module_api('Tension Member Welded Connection')
        input_values = request.data
        tempData = {
            'cookie_id': cookie_id,
            'module_id': 'Tension Member Welded Connection',
