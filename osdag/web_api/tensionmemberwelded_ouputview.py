...
class TensionMemberWeldedOutputData(APIView):
    def post(self, request):
        ...
        cookie_id = request.COOKIES.get('tension_member_welded_design_session')
        print('in tension_outputView.py: cookie id in tension member welded design output data ', cookie_id)
        module_api = get_module_api('Tension Member Welded Design')
        input_values = request.data
        tempData = {
            'cookie_id': cookie_id,
            'module_id': 'Tension Member Welded Design',
