...
from osdag_api.modules.tension_member_welded import *
...
class TensionMemberWeldedOutputData(APIView):
    ...
    def post(self, request):
        print("Inside post method of TensionMemberWeldedOutputData")

        # obtaining the session, module_id, input_values
        cookie_id = request.COOKIES.get('tension_member_welded')
        module_api = get_module_api('Tension Member Welded')
        input_values = request.data
        tempData = {
            'cookie_id': cookie_id,
            'module_id': 'Tension Member Welded',
